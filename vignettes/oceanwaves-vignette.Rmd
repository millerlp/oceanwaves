...
The `prCorr` function needs a few arguments to work. In addition to the vector of sea surface heights (in meters), you also need to provide the sampling frequency for the data logger (4 Hz in our `wavedata` example dataset), and the height of the data logger above the sea floor. 
...
plot(x = wavedata$DateTime, y = swDepthCorrected.m, type = 'l', col = 'red', ylab = 'Surface height, m')
lines(x = wavedata$DateTime, y = swDepth.m, col = 'blue')
legend('topright',legend=c('Corrected','Raw'), col = c('red','blue'),
       lty = 1)
```

As the comparison figure shows, the correction in surface height is not
...
You may also notice the declining trend in the mean height of the sea surface. This is due to the falling tide during this collection interval. The calculations of wave statistics below do remove this trend before attempting to calculate the wave heights and periods.
...
height data. We will use two different approaches to arrive at roughly the same end results: estimates of significant wave height and wave period.
...
The `oceanwaves` package implements an up-crossing algorithm in the function `waveStatsZC`, which should be supplied with a vector of water surface heights and the sampling frequency of the data (4 Hz for our 
example `wavedata`).
